/*
 Biography Platform - Full Schema
 Updated: 2026-07-01
 MySQL 8.0+

 Changes from original:
   - biography: added owner_id, comment_num, likes
   - bio_comment: rewritten to match Java entity (user_id, parent_id, comment_body, comment_status)
   - Added is_deleted to user_role and role tables
*/
...
INSERT INTO `role` VALUES (1, 'ROLE_ADMIN',   0, '2020-10-26 21:09:32', '2020-10-26 21:09:32');
INSERT INTO `role` VALUES (2, 'ROLE_USER',    0, '2020-10-26 21:09:53', '2020-10-26 21:09:53');
INSERT INTO `role` VALUES (3, 'ROLE_COMPANY', 0, '2020-10-26 21:10:07', '2020-10-26 21:10:07');
...
(
    ...
    `is_locked`       tinyint UNSIGNED                                        NOT NULL DEFAULT 0 COMMENT '0-未锁定，1-已锁定（注册时锁定，邮件激活后解锁）',
    `is_deleted`      tinyint UNSIGNED                                        NOT NULL DEFAULT 0 COMMENT '0-未删除，1-已删除',
    `gmt_create`      datetime(0)                                             NOT NULL DEFAULT CURRENT_TIMESTAMP(0) COMMENT '记录创建时间',
    `gmt_modified`    datetime(0)                                             NOT NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '记录修改时间',
    PRIMARY KEY (`id`) USING BTREE,
...
(
    `id`          bigint UNSIGNED                                         NOT NULL COMMENT '激活令牌id',
    `expiry_date` datetime(0)                                             NOT NULL COMMENT '过期时间',
    `token`       varchar(225) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '令牌值',
...
(
    ...
    `user_id`        bigint UNSIGNED NOT NULL COMMENT '关联用户表id',
    `bio_num`        int UNSIGNED    NOT NULL DEFAULT 0 COMMENT '发表传记数量',
    ...
    `gmt_modified`   datetime(0)     NOT NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '记录修改时间',
    PRIMARY KEY (`id`) USING BTREE,
    UNIQUE INDEX `user_active_user_id_uindex` (`user_id`) USING BTREE,
...
(
    ...
    `user_group`   enum ('GROUP_FRIEND','GROUP_RELATIVE') CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT 'GROUP_FRIEND' COMMENT '用户对好友的分组',
    `friend_group` enum ('GROUP_FRIEND','GROUP_RELATIVE') CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT 'GROUP_FRIEND' COMMENT '朋友对用户的分组',
    `is_deleted`   tinyint UNSIGNED                                                                  NOT NULL DEFAULT 0 COMMENT '0-未删除，1-已删除',
    `gmt_create`   datetime(0)                                                                       NOT NULL DEFAULT CURRENT_TIMESTAMP(0) COMMENT '记录创建时间',
    `gmt_modified` datetime(0)                                                                       NOT NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '记录修改时间',
...
-- ----------------------------
-- Table structure for biography  (fixed: added owner_id, comment_num, likes)
-- ----------------------------
DROP TABLE IF EXISTS `biography`;
CREATE TABLE `biography`
(
    `id`             bigint UNSIGNED                                         NOT NULL COMMENT '传记id',
    `owner_id`       bigint UNSIGNED                                         NOT NULL COMMENT '作者用户id',
    ...
    `category_name`  varchar(45)  CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '传记所属类别名称(冗余字段)',
    ...
    `comment_num`    bigint UNSIGNED                                         NOT NULL DEFAULT 0 COMMENT '评论数量',
    `likes`          bigint UNSIGNED                                         NOT NULL DEFAULT 0 COMMENT '点赞数量',
    `is_deleted`     tinyint UNSIGNED                                        NOT NULL DEFAULT 0 COMMENT '0-未删除，1-已删除',
    `gmt_create`     datetime(0)                                             NOT NULL DEFAULT CURRENT_TIMESTAMP(0) COMMENT '记录创建时间',
    `gmt_modified`   datetime(0)                                             NOT NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '记录修改时间',
    PRIMARY KEY (`id`) USING BTREE,
    INDEX `fk_biography_category_idx` (`category_id`) USING BTREE,
    INDEX `idx_biography_owner_id` (`owner_id`) USING BTREE,
    ...
    CONSTRAINT `fk_biography_owner`    FOREIGN KEY (`owner_id`)    REFERENCES `user` (`id`)         ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB
  CHARACTER SET = utf8
  COLLATE = utf8_general_ci
  ROW_FORMAT = Dynamic;
...
(
    ...
    `tag_name`     varchar(45) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '标签名',
    `is_deleted`   tinyint UNSIGNED                                       NOT NULL DEFAULT 0 COMMENT '0-未删除，1-已删除',
    `gmt_create`   datetime(0)                                            NOT NULL DEFAULT CURRENT_TIMESTAMP(0) COMMENT '记录创建时间',
    `gmt_modified` datetime(0)                                            NOT NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '记录修改时间',
    PRIMARY KEY (`id`) USING BTREE,
    UNIQUE INDEX `tag_name_UNIQUE` (`tag_name`) USING BTREE
) ENGINE = InnoDB
  CHARACTER SET = utf8
  COLLATE = utf8_general_ci
  ROW_FORMAT = Dynamic;
...
(
    `bio_id`       bigint UNSIGNED NOT NULL COMMENT '传记id',
    `tag_id`       bigint UNSIGNED NOT NULL COMMENT '标签id',
    `gmt_create`   datetime(0)     NOT NULL DEFAULT CURRENT_TIMESTAMP(0) COMMENT '记录创建时间',
    `gmt_modified` datetime(0)     NOT NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '记录修改时间',
    PRIMARY KEY (`bio_id`, `tag_id`) USING BTREE,
    INDEX `fk_bio_tag_tag_idx` (`tag_id`) USING BTREE,
    CONSTRAINT `fk_bio_tag_bio` FOREIGN KEY (`bio_id`) REFERENCES `biography` (`id`) ON DELETE CASCADE ON UPDATE RESTRICT,
...
-- ----------------------------
-- Table structure for bio_comment  (rewritten: user_id + parent_id, removed old fields)
-- ----------------------------
...
(
    ...
    `comment_body`   varchar(200) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '评论内容',
    `comment_status` tinyint UNSIGNED                                        NOT NULL DEFAULT 0 COMMENT '0-未审核，1-审核通过',
    `parent_id`      bigint UNSIGNED                                         NULL     DEFAULT NULL COMMENT '父评论id（为null表示顶级评论）',
    `is_deleted`     tinyint UNSIGNED                                        NOT NULL DEFAULT 0 COMMENT '0-未删除，1-已删除',
    `gmt_create`     datetime(0)                                             NOT NULL DEFAULT CURRENT_TIMESTAMP(0) COMMENT '评论创建时间',
    `gmt_modified`   datetime(0)                                             NOT NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '记录修改时间',
    PRIMARY KEY (`id`) USING BTREE,
    INDEX `idx_bio_comment_bio_id`    (`bio_id`)    USING BTREE,
    INDEX `idx_bio_comment_user_id`   (`user_id`)   USING BTREE,
    INDEX `idx_bio_comment_parent_id` (`parent_id`) USING BTREE,
    CONSTRAINT `fk_bio_comment_bio`  FOREIGN KEY (`bio_id`)   REFERENCES `biography` (`id`) ON DELETE CASCADE ON UPDATE RESTRICT,
    CONSTRAINT `fk_bio_comment_user` FOREIGN KEY (`user_id`)  REFERENCES `user`      (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB
  CHARACTER SET = utf8
  COLLATE = utf8_general_ci
  ROW_FORMAT = Dynamic;
